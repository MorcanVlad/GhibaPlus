...
export default function Dashboard() {
  ...
  // Referințe pentru a opri Firebase la Logout
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = auth.onAuthStateChanged(async (u) => {
      ...
      if (snap.exists()) {
        ...
        setUser(userData); setEditPhone(userData.phone || ""); 
        setCurrentLang(userData.language || "ro"); setEditLang(userData.language || "ro");
  ...
  // Funcția sigură de LOGOUT care previne erorile
  ...
  useEffect(() => {
      ...
      calItems = calItems.filter((item:any) => new Date(item.endDate || item.date) >= today);
  ...
  // Traducere SIGURĂ care previne Crash-ul și eroarea 400
  const translateText = async (text: string, targetLang: string) => {
    ...
    try {
      ...
      const translated = data.translatedText || text; // Fallback instant dacă e gol
      
      translationCache.current.set(cacheKey, translated);
      return translated;
    } catch (error) { 
      return text; // Fallback pe limba nativă
    }
  };

  useEffect(() => {
    const translateWholePage = async () => {
      if (currentLang === 'ro' || (feed.length === 0 && calendarEvents.length === 0)) {
    ...
    translateWholePage();
  }, [feed, calendarEvents, currentLang]);
  ...
  const handleRegisterClick = async (e: any, item: any) => {
      ...
      if (item.isTeamEvent) {
          ...
          if (!isReg) {
              await sendNotification("Înscriere Confirmată ✅", `Te-ai înscris cu succes la: ${item.title}`);
          } else {
              await sendNotification("Participare Anulată ❌", `Te-ai retras de la: ${item.title}`);
          }
      }
  };
  ...
  const handleTransferLeadership = async (eventId: string, team: any, newLeader: any) => {
      ...
      const updatedTeam = {
          ...team,
          leaderId: newLeader.id, leaderName: newLeader.name, leaderClass: newLeader.class, leaderPhone: newLeader.phone,
      ...
      {teamModalEvent && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className={`w-full max-w-lg p-8 sm:p-10 rounded-[2.5rem] border shadow-2xl relative animate-popup ${cardBg}`}>
            ...
            <h2 className="text-xl font-black mb-2 flex items-center gap-2 text-green-500">Formează Echipa</h2>
            <p className="text-xs opacity-70 mb-6">{teamModalEvent.title} • {teamMembers.length + 1} / {teamModalEvent.teamSize} Membri</p>
      ...
      {manageTeamModal && (() => {
          ...
          <div className="fixed inset-0 z-[75] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
              <div className={`w-full max-w-lg p-8 sm:p-10 rounded-[2.5rem] border shadow-2xl relative animate-popup ${cardBg}`}>
                  ...
                  <h2 className="text-xl font-black mb-2 flex items-center gap-2 text-blue-500">Gestionează Echipa</h2>
                  <p className="text-xs opacity-70 mb-6">{manageTeamModal.title} • Lider: {myTeam.leaderName}</p>
